...
public class EnsembleCacheManager implements  BasicCacheContainer{
    ...
    private Map<Site,RemoteCacheManager> managers;
    ...
    public EnsembleCacheManager(List<String> sites, String zkHost, int to) throws CacheException{
        ...
        try {
            ...
            this.index = new ZkHashMap<String, List<Site>>(ZK_INDEX,zkManager,new zkSerializer<Map.Entry<String, List<Site>>>());
            this.sites = new ZkListSet<Site>(ZK_SITES,zkManager,new zkSerializer<Site>());
        } catch (Exception e) {
        ...
        this.managers = new HashMap<Site, RemoteCacheManager>();
    ...
    public synchronized <K,V> BasicCache<K,V> getCache(String cacheName, List<Site> sites, Consistency consistency) {
        ...
        for(Site ucloud: sites){
            RemoteCache<K,V> c = (RemoteCache<K,V>) ucloud.getManager().getCache(cacheName);
            caches.add(c);
        }
    ...
    public synchronized void addSite(Site site){
        sites.add(site);
        managers.put(site, site.getManager());
    }
    ...
    private List<Site> assignRandomly(int replicationFactor){
        List<Site> sites = new ArrayList<Site>(managers.keySet());
        java.util.Collections.shuffle(sites);
        for(int i=sites.size()-replicationFactor;i>0;i--)
            sites.remove(0);
        return sites;
    }
